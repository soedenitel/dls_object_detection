...
В силу того, что дежурный видеопульта страдает притуплением внимания из-за того, что большую часть времени ничего не происходит, предлагаю следующее решение:
> - разработать отдельный сервис по графической разметке;
> - детектор будет собирать исходные изображения, у которых детектированы целевые объекты с вероятностью в заданном диапазоне (не ниже и не выше некоторых пороговых значений) и передовать их в сервис из предыдущего пункта;
> - дежурному видеопульта поставить задачу размечать в разработанном сервисе объекты прямоугольниками, которые ему нужны для реагирования;
> - по размеченым изображениям проводить регулярное дообучение модели.